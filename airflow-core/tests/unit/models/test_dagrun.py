...
class TestDagRun:
    ...
    @mock.patch.object(Variable, "get", side_effect=KeyError("missing"))
    @mock.patch.object(Deadline, "prune_deadlines")
    def test_dagrun_deadline_variable_interval_missing_variable_fails(
        self, _, mock_get, session, deadline_test_dag
    ):
        future_date = datetime.datetime.now() + datetime.timedelta(days=365)
