...
def upgrade():
    ...
    with op.batch_alter_table("dag_run", schema=None) as batch_op:
        batch_op.create_index("idx_created_dag_version_id", ["created_dag_version_id"], unique=False)
...
def downgrade():
    ...
    with op.batch_alter_table("task_instance") as batch_op:
        if conn.dialect.name == "mysql":
            batch_op.drop_constraint("task_instance_dag_version_id_fkey", type_="foreignkey")
            batch_op.drop_index("ti_dag_version_id")
            batch_op.create_foreign_key(
                "task_instance_dag_version_id_fkey",
                "dag_version",
                ["dag_version_id"],
                ["id"],
                ondelete="RESTRICT",
    ...
    with op.batch_alter_table("dag_run") as batch_op:
        if conn.dialect.name == "mysql":
            ...
            batch_op.drop_index("idx_created_dag_version_id")
            batch_op.create_foreign_key(
