...
test.describe("Variables Page", () => {
  ...
  // Pre-registered so afterAll can clean up even if a test fails before pushing into createdVariables.
  const importVarKeys: Array<string> = [];
  const bulkDeleteKeys: Array<string> = [];
  ...
  test("verify bulk deleting multiple variables", async ({ page, variablePage }) => {
    const bulkPrefix = uniqueRunId("bulk_del");
    const bulkKeys = [`${bulkPrefix}_1`, `${bulkPrefix}_2`, `${bulkPrefix}_3`];

    bulkDeleteKeys.push(...bulkKeys);

    for (const key of bulkKeys) {
      await apiCreateVariable(page, { key, value: `val_${key}` });
    }

    await variablePage.navigate();
    await variablePage.waitForLoad();

    await variablePage.search(bulkPrefix);
    await expect(variablePage.tableRows).toHaveCount(3, { timeout: 10_000 });

    for (const key of bulkKeys) {
      await variablePage.selectRow(key);
    }
    ...
    await expect(
      dialog.getByRole("heading", { name: new RegExp(`delete\\s+${bulkKeys.length}\\s+variable`, "i") }),
    ).toBeVisible();
    ...
    for (const key of bulkKeys) {
      await expect(variablePage.rowByKey(key)).toHaveCount(0);
    }
  });
  ...
  test.afterAll(async ({ authenticatedRequest }) => {
    ...
    const allKeys = [
      ...new Set([...createdVariables.map((v) => v.key), ...importVarKeys, ...bulkDeleteKeys]),
    ];
