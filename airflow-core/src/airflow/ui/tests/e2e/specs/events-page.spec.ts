...
test.describe("Events Page", () => {
  ...
  test("verify search input is visible", async ({ eventsPage }) => {
    ...
    const filterButton = eventsPage.page.getByTestId("add-filter-button");

    await expect(filterButton).toBeVisible();
    await filterButton.click();

    const filterMenu = eventsPage.page.getByRole("menu");

    await expect(filterMenu).toBeVisible({ timeout: 5000 });

    const textSearchOptions = eventsPage.page.getByRole("menuitem").filter({
