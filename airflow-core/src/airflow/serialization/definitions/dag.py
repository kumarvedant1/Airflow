...
class SerializedDAG:
    ...
    def set_multiple_task_instances_state(
        ...
        *,
        ...
        """
        Set the state of multiple TaskInstances and clear their downstream tasks.

        Batch version of :meth:`set_task_instance_state` that issues a single
        query instead of one per task, avoiding N+1 query overhead for task groups.

        :param task_ids_with_map_indexes: List of ``(task_id, map_index)`` tuples
        :param run_id: The run_id of the DagRun
        :param state: State to set the TaskInstances to
        :param upstream: Include all upstream tasks
        :param downstream: Include all downstream tasks
        :param future: Include all future TaskInstances
        :param past: Include all past TaskInstances
        :param commit: Commit changes
        :param session: database session
        """
        from airflow.api.common.mark_tasks import set_state
        ...
        task_cache: dict[str, SerializedOperator] = {}

        for task_id, map_index in task_ids_with_map_indexes:
            if task_id not in task_cache:
                task = self.get_task(task_id)
                task.dag = self
                task_cache[task_id] = task
            tasks_to_set_state.append((task_cache[task_id], map_index))
        ...
        clear_kwargs: dict = {
            "only_failed": True,
            "session": session,
            "exclude_task_ids": frozenset(unique_task_ids),
        }
