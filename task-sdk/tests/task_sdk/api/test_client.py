...
class TestAssetEventOperations:
    ...
    def test_partition_key_param_passed(self):
        def handle_request(request: httpx.Request) -> httpx.Response:
            params = request.url.params
            assert params.get("partition_key_pattern") == "^2024-01-01$"
            return httpx.Response(
                status_code=200,
                json={
                    "asset_events": [
    ...
    def test_partition_key_with_other_params(self):
        ...
        result = client.asset_events.get(
            name="this_asset",
            partition_key=r"^us\|2024-.*",
            after=datetime(2023, 6, 1, tzinfo=timezone.utc),
            limit=5,
            ascending=False,
        )
    ...
    def test_partition_key_with_alias(self):
        def handle_request(request: httpx.Request) -> httpx.Response:
            ...
            assert params.get("partition_key_pattern") == r"^us-east\|.*"
            return httpx.Response(
                status_code=200,
                json={"asset_events": []},
            )
        ...
        client = make_client(httpx.MockTransport(handle_request))
        result = client.asset_events.get(alias_name="my_alias", partition_key=r"^us-east\|.*")
